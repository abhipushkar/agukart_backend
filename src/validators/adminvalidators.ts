...
export const categoryValid = Joi.object({
  ...
  conditionType: Joi.string().valid("all", "any").optional().default("all"),
  ...
  selectedCategories: Joi.array().items(objectId).optional().default([]),
  img_dimension: Joi.string().valid("2x3", "4x3").optional().default("2x3"),
});
...
export const informationValid = Joi.object({
  ...
  meta_keywords: Joi.alternatives()
...
export const createStoreValid = Joi.object({
  ...
  selected_sku_codes: Joi.alternatives().conditional("product_select", {
    ...
    then: Joi.array()
      ...
      .messages({
        "array.min":
          'At least one SKU code is required when product_select is "Products SKU"',
      }),
  ...
  selected_products: Joi.alternatives().conditional("product_select", {
    ...
    then: Joi.array()
      ...
      .messages({
        "array.min":
          'At least one product is required when product_select is "Products Name"',
      }),
    otherwise: Joi.forbidden(),
  }),
